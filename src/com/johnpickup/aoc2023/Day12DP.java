...
public class Day12DP {
    public static void main(String[] args) {
        ...
        try (Stream<String> stream = Files.lines(Paths.get("/Volumes/Users/john/Development/AdventOfCode/resources/2023/Day12.txt"))) {
            ...
            long solution = springLines.stream().map(Springs::combinations).reduce(0L, Long::sum);
    ...
    static class Springs {
        ...
        public long combinations(boolean inGroup) {
            if (cache.containsKey(new Key(this, inGroup))) {
                return cache.get(new Key(this, inGroup));
            }
                ...
                case BROKEN:
                    ...
                    if (brokenGroups.get(0) == 1) {
                        ...
                        if (!nextStates.isEmpty() && nextStates.get(0) == State.UNKNOWN) {
                            ...
                            result = new Springs(statesWithOK, newBrokenGroups).combinations(false);
                        }
                    }
                    else {
                        ...
                        if (!nextStates.isEmpty() && nextStates.get(0) == State.UNKNOWN) {
                            ...
                            newBrokenGroups.addAll(brokenGroups.subList(1, brokenGroups.size()));
                            result = new Springs(statesWithBroken, newBrokenGroups).combinations(true);
                        }
                        newBrokenGroups.add(brokenGroups.get(0) - 1);
                    }
                    if (result == null) {
            ...
            cache.put(new Key(this, inGroup), result);
            return result;
        }
    ...
    static final Map<Key, Long> cache = new HashMap<>();
