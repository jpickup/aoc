...
public class Day22Reactor2D {
    ...
    private long countCubes(Set<Coord> reactor, int minX, int maxX, int minY, int maxY) {
        return reactor.stream().filter(c -> c.inBounds(minX, maxX, minY, maxY)).count();
    }
        ...
        for (int x = instruction.minX; x <= instruction.maxX; x++) {
            for (int y = instruction.minY; y <= instruction.maxY; y++) {
                    if (instruction.operation) {
                        reactor.add(new Coord(x,y));
                    }
                    else {
                        reactor.remove(new Coord(x,y));
                    }
            }
        }
    }
    ...
    private Instruction parse(String s) {
        ...
        return new Instruction(op, parseMin(parts[0]), parseMax(parts[0]), parseMin(parts[1]), parseMax(parts[1]));
    }
    ...
    public Day22Reactor2D(String filename) {
    ...
    static class Instruction {
        ...
        public Instruction(boolean operation, int minX, int maxX, int minY, int maxY) {
