...
public abstract class DijkstraWithoutFullState<State> {

    protected abstract Set<State> adjacentStates(State state);
    ...
    private void generatePaths() {
        ...
        while (lowestCostEntry != null && lowestCostEntry.getValue() < Long.MAX_VALUE) {
            // build the unvisited states incrementally. For when the state space is too large
            for (State adjacentState : adjacentStates(lowestCostEntry.getKey())) {
                if (!visited.containsKey(adjacentState)) unvisited.putIfAbsent(adjacentState, Long.MAX_VALUE);
            }
//      System.out.println("Current  : " + lowestCostEntry);
